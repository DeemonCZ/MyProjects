...
*/
    ...
    if (s != NULL) {        //pokud předávaný parametr "s" není NULL,
        s->top = -1;        //nastaví vrchol zísobníku na -1, tudíž prázdný zásobník
    } else {
...
*/
    return (s->top == -1);      //pokud je seznam prázdný, vrací nenulovou hodnotu
}
...
*/
    return (s->top+1 == STACK_SIZE);    //pokud je seznam plný, vrací nenulovou hodnotu
}
...
*/
    ...
    if (stackEmpty(s)){             //když zásobník neni prázdný,
        stackError(SERR_TOP);       //vrátí hodnotu z vrcholi zásobníku
    }
    else {
        *c = s->arr[s->top];
    }
...
*/
    if (!stackEmpty(s)) {       //když zásobník neni prázdný,
        s->top = s->top-1;      //odstraní prvek z vrcholu zásobníku
    }
}
...
** funkci stackFull.
*/
    if (stackFull(s)) {             //pokud je to možné, vloží prvek na vrchol zásobníku
